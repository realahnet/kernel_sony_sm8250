			    ...
			    struct common_audit_data *a)
{
	...
	rc = security_sid_to_context(state, ssid, &scontext,
				     &scontext_len);
	rc = security_sid_to_context(state, tsid, &tcontext,
				     &tcontext_len);
	tclass_s = secclass_map[sad.tclass-1].name;
	common_lsm_audit(a, avc_audit_pre_callback, avc_audit_post_callback);
	trace_selinux_audited(&sad, scontext, tcontext, tclass_s);
	kfree(tcontext);
	kfree(scontext);
